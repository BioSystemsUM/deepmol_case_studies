...
def run():
    ...
    train_dataset = CSVLoader(dataset_path=os.path.join(current_directory, "train.csv"), 
                              smiles_field="smiles",labels_fields=["true_labels_R"]).create_dataset()
    ...
    pipeline.optimize(train_dataset = train_dataset, test_dataset=validation_dataset, objective_steps=steps, n_trials=20, save_top_n=10,
...
def test_best_pipeline():
    ...
    pipeline = VotingPipeline.load("del/voting_pipeline")
